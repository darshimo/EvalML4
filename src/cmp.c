...
int cmpClsr(Clsr *ob1, Clsr *ob2)
{
    ...
    printf(" :\n");
#endif
    if (cmpEnv(ob1->env_, ob2->env_))
        return 1;
    if (cmpVar(ob1->arg, ob2->arg))
        return 1;
...
int cmpConsv(Consv *ob1, Consv *ob2)
{
    ...
    printf(" :\n");
#endif
    if (cmpVal(ob1->val1_, ob2->val1_))
        return 1;
    if (cmpVal(ob1->val2_, ob2->val2_))
        return 1;
    return 0;
}
...
int cmpEnv(Env *ob1, Env *ob2)
{
    ...
    if (ob1 == NULL && ob2 == NULL)
        return 0;
    if (cmpVar(ob1->var_, ob2->var_))
        return 1;
    if (cmpVal(ob1->val_, ob2->val_))
        return 1;
    if (cmpEnv(ob1->prev, ob2->prev))
        return 1;
    return 0;
}
...
{
    if (ob1->val_type != ob2->val_type)
        return 1;
    if (ob1->val_type == INT_)
        return cmpInt(ob1->u.int_, ob2->u.int_);
    if (ob1->val_type == BOOL_)
        return cmpBool(ob1->u.bool_, ob2->u.bool_);
    if (ob1->val_type == CLSR)
        return cmpClsr(ob1->u.clsr_, ob2->u.clsr_);
    if (ob1->val_type == CLSRREC)
        cmpClsrRec(ob1->u.clsrrec_, ob2->u.clsrrec_);
    if (ob1->val_type == CONS_)
        cmpConsv(ob1->u.consv_, ob2->u.consv_);
    return 0;
}
...
{
    if (ob1->exp_type != ob2->exp_type)
        return 1;
    if (ob1->exp_type == INT)
        return cmpInt(ob1->u.int_, ob2->u.int_);
    if (ob1->exp_type == BOOL)
        return cmpBool(ob1->u.bool_, ob2->u.bool_);
    if (ob1->exp_type == VAR)
        return cmpVar(ob1->u.var_, ob2->u.var_);
    if (ob1->exp_type == OP)
        return cmpOp(ob1->u.op_, ob2->u.op_);
    if (ob1->exp_type == IF)
        return cmpIf(ob1->u.if_, ob2->u.if_);
    if (ob1->exp_type == LET)
        return cmpLet(ob1->u.let_, ob2->u.let_);
    if (ob1->exp_type == FUN)
        return cmpFun(ob1->u.fun_, ob2->u.fun_);
    if (ob1->exp_type == APP)
        return cmpApp(ob1->u.app_, ob2->u.app_);
    if (ob1->exp_type == LETREC)
        cmpLetRec(ob1->u.letrec_, ob2->u.letrec_);
    if (ob1->exp_type == CONS)
        cmpConse(ob1->u.conse_, ob2->u.conse_);
    if (ob1->exp_type == MATCH)
